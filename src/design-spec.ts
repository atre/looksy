...
export function loadDesignSpec(specPath: string): DesignSpec {
  ...
  if (data.fonts && typeof data.fonts !== 'object')
    throw new Error('Design spec "fonts" must be an object');
  if (data.colors && typeof data.colors !== 'object')
    throw new Error('Design spec "colors" must be an object');
  if (data.spacing && typeof data.spacing !== 'object')
    throw new Error('Design spec "spacing" must be an object');
  return data;
}
  ...
  if (spec.colors) {
    for (const [key, expected] of Object.entries(spec.colors)) {
      ...
      checks.push({
        ...
        property,
        expected,
        assertion: `${property} ${selector} = ${expected}`,
      });
    }
  }
  ...
  if (spec.spacing) {
    for (const [key, expected] of Object.entries(spec.spacing)) {
      ...
      checks.push({
        ...
        property,
        expected,
        assertion: `${selector} ${property} = ${expected}`,
      });
    }
  }
  ...
  const results = await page.evaluate((checksData: Check[]) => {
    ...
    const $ = {
      ...
      rgbToHex(color: string): string {
        const n = $.norm(color);
        if (!n) return color;
        const hexAlready = n.match(/^#([0-9a-f]{2})([0-9a-f]{2})([0-9a-f]{2})/i);
        if (hexAlready) return n.slice(0, 7);
...
export function formatDesignValidation(
