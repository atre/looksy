...
async function runCaptureFlow(
  ...
  const both = values.both ?? false;
  try {
    assertNotBothAndDark(both, darkMode);
  } catch (err: any) {
  ...
  const quality = values.quality
    ...
    : undefined;
  const thumb = values.thumb ? validateNumeric('thumb', values.thumb) : undefined;
  if (thumb !== undefined && thumb <= 0) {
    console.error('looksy: --thumb must be a positive integer');
    process.exit(1);
  }
  const crop = values.crop
    ? (() => {
        try {
          return parseCrop(values.crop!);
        } catch (err: any) {
          console.error(`looksy: ${err.message}`);
          process.exit(1);
        }
  ...
  try {
    if (values.multi) {
      ...
      const base = suffix ? applySuffix(rawBase, suffix) : rawBase;
      const stem = base.replace(/\.(png|jpg|jpeg)$/, '');
      const ext = format === 'jpeg' ? '.jpg' : '.png';
      const { browser } = await connectOrLaunch({ hostResolverRule });
      try {
        const [light, dark] = await Promise.all([
          screenshot({ ...buildConfig(`${stem}-light${ext}`), browser, darkMode: false }),
          screenshot({ ...buildConfig(`${stem}-dark${ext}`), browser, darkMode: true }),
        ]);
        if (values.brief) {
          printBrief([
            { url: `${url} (light)`, result: light },
            { url: `${url} (dark)`, result: dark },
          ]);
        } else {
          printResult(light, { quiet: values.quiet });
          printResult(dark, { quiet: values.quiet });
        }
        contrastExit([light, dark]);
      } finally {
        await browser.close();
      }
    } else {
...
async function runUrlsBatch(
  ...
  const items: Array<{ url: string; scheme?: 'light' | 'dark' }> = values.both
    ? urlList.flatMap((u) => [
        { url: u, scheme: 'light' as const },
        { url: u, scheme: 'dark' as const },
      ])
    : urlList.map((u) => ({ url: u }));
  ...
  try {
    const settled = await pMapSettled(
      items,
      async ({ url: pageUrl, scheme }) => {
        const safeSuffix = scheme
          ? `${urlToOutputSuffix(pageUrl)}-${scheme}`
          : urlToOutputSuffix(pageUrl);
        const baseOutput = outputDir
    ...
    settled.forEach((s, i) => {
      ...
      else failures.push({ target: items[i].url, message: s.error.message });
    });
