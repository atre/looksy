...
export async function screenshot(config: ScreenshotConfig): Promise<ScreenshotResult> {
  ...
  try {
    ...
    if (config.thumb && !textOnly && !config.pdf) {
      const { width: tw, height: th } = thumbDimensions(config.width, config.height, config.thumb);
      const thumbPath = outputPath.replace(/\.(png|jpg|jpeg)$/, '.thumb.jpg');
