...
import type { FailedRequest } from './failed-requests.js';
...
export const CHECK_ASSERTIONS: Array<{ syntax: string; description: string }> = [
  ...
  {
    syntax: 'no-hscroll',
    description: 'page is not wider than the viewport (no horizontal scroll)',
  },
  {
  ...
  {
    syntax: 'no-broken-images',
    description: 'no <img> failed to load (lazy not-yet-loaded is not broken)',
  },
  { syntax: 'status:<code>', description: 'main document HTTP status equals <code>' },
  {
    syntax: 'assets-ok',
    description: 'no same-origin css/js/img/font request failed or returned ≥ 400',
  },
  ...
  {
    syntax: 'og-image | og-title | og-tags',
    description: 'Open Graph image / title / title+description+image present',
  },
...
export function isKnownAssertion(assertion: string): boolean {
  ...
  if (/^status:\d{3}$/.test(lower)) return true;
  return false;
}
...
export function evaluateStatusAssertion(
  assertion: string,
  httpStatus: number | undefined,
): CheckResult {
  const want = Number(assertion.trim().slice('status:'.length));
  if (httpStatus === undefined)
    return { assertion, pass: false, detail: 'no response status recorded' };
  return httpStatus === want
    ? { assertion, pass: true, detail: `HTTP ${httpStatus}` }
    : { assertion, pass: false, detail: `got ${httpStatus}` };
}

export function evaluateAssetsOk(failed: FailedRequest[], assertion = 'assets-ok'): CheckResult {
  if (failed.length === 0) return { assertion, pass: true, detail: 'no failed asset requests' };
  return {
    assertion,
    pass: false,
...
export async function runChecks(
  page: Page,
  assertions: string,
  opts: { contrastLimit?: number; httpStatus?: number; failedRequests?: FailedRequest[] } = {},
): Promise<string> {
...
export async function runChecksStructured(
  page: Page,
  assertions: string,
  opts: { contrastLimit?: number; httpStatus?: number; failedRequests?: FailedRequest[] } = {},
  ...
  const fontCheckItems = items.filter((item) =>
    FONT_CHECK_ASSERTIONS.has(item.toLowerCase().trim()),
  );
      ...
      if (lower === 'self-hosted-fonts') {
        if (fontData.externalDomains.length === 0) {
          asyncResults.push({
            assertion,
            pass: true,
            detail: `all ${fontData.sources.length} font(s) self-hosted`,
          });
        } else {
          asyncResults.push({
            assertion,
            pass: false,
            detail: `external font domain(s) found: ${fontData.externalDomains.join(', ')}`,
          });
        }
        ...
        if (googleDomains.length === 0 && googleSources.length === 0) {
          ...
          asyncResults.push({
            assertion,
            pass: false,
            detail: `Google Fonts detected: ${domains.join(', ')}`,
          });
        }
      }
    }
  ...
  const structuralCheckItems = items.filter((item) =>
    STRUCTURAL_CHECK_ASSERTIONS.has(item.toLowerCase().trim()),
  );
      ...
      if (lower === 'unique-footer') {
        if (structuralData.footerHtml === null) {
          ...
          asyncResults.push({
            assertion,
            pass: true,
            detail: `<footer> present (${structuralData.footerByteLength} bytes) — use with --pages for cross-page comparison`,
          });
        }
        ...
        if (structuralData.navHtml === null) {
          ...
          asyncResults.push({
            assertion,
            pass: true,
            detail: `<nav> present (${structuralData.navByteLength} bytes) — use with --pages for cross-page comparison`,
          });
        }
      }
    }
  ...
  const domItems = items.filter((item) => {
    const l = item.toLowerCase().trim();
    return !ASYNC_ASSERTIONS.has(l) && !isNodeSide(l);
  });

  const domResults: CheckResult[] =
    domItems.length > 0
      ? await page.evaluate(
          (evalArgs: { checks: string[]; contrastLimit: number }) => {
            ...
            const $ = {
              ...
              matchColor(
                ...
                const pass =
                  Math.abs(r - exp[0]) <= tolerance &&
                  Math.abs(g - exp[1]) <= tolerance &&
                  Math.abs(b - exp[2]) <= tolerance;
            ...
            return checks.map((assertion): { assertion: string; pass: boolean; detail: string } => {
              ...
              if (lower.includes('sticky') && lower.includes('header')) {
                ...
                return {
                  assertion,
                  pass: pos === 'sticky' || pos === 'fixed',
                  detail: `header position: ${pos}`,
                };
              }
              ...
              if (lower === 'no generator') {
                ...
                if (!gen)
                  return { assertion, pass: true, detail: 'no <meta name="generator"> found' };
                return {
                  assertion,
                  pass: false,
                  detail: `generator: "${gen.getAttribute('content')}"`,
                };
              }
                ...
                try {
                  ...
                  if (found === 0) {
                    ...
                    return {
                      assertion,
                      pass: styleMatch === 0,
                      detail: `${styleMatch} element(s) with "${pattern}" in style`,
                    };
                  }
                  return {
                    assertion,
                    pass: false,
                    detail: `${found} element(s) with "${pattern}" in class`,
                  };
                } catch {
              ...
              if (lower.startsWith('text:')) {
                ...
                return {
                  assertion,
                  pass: !!found,
                  detail: found ? 'found in page text' : 'not found in page text',
                };
              }
              ...
              if (lower.startsWith('font:') && assertion.includes('=')) {
                ...
                try {
                  ...
                  const pass =
                    primary.toLowerCase() === expected.toLowerCase() ||
              ...
              if (lower.startsWith('bg:') && assertion.includes('=')) {
                ...
                try {
                  ...
                  return {
                    assertion,
                    pass: result.match,
                    detail: `bg: ${result.actual} (expected ${expected})`,
                  };
                } catch {
                  return { assertion, pass: false, detail: 'invalid selector' };
                }
              ...
              if (
                ...
                try {
                  ...
                  return {
                    assertion,
                    pass: result.match,
                    detail: `color: ${result.actual} (expected ${expected})`,
                  };
                } catch {
                  return { assertion, pass: false, detail: 'invalid selector' };
                }
              ...
              if (lower === 'contrast:aa' || lower === 'contrast:aaa') {
                ...
                const cc = {
                  ...
                  lum(r: number, g: number, b: number) {
                    return (
                      0.2126 * cc.toLinear(r) + 0.7152 * cc.toLinear(g) + 0.0722 * cc.toLinear(b)
                    );
                  },
                ...
                for (const el of document.querySelectorAll(
                  ...
                  if (ratio < threshold) {
                    ...
                    failingElements.push(
                      `${tag}${cls ? '.' + cls : ''} "${text}" ${ratio.toFixed(1)}:1`,
                    );
                  }
                }

                const capNote = capped
                  ? ` (sampled ${checked}/${eligible}; ${Math.max(0, eligible - checked)} unchecked — raise with --contrast-limit)`
                  : '';
                if (failingElements.length === 0) {
                  return {
                    assertion,
                    pass: true,
                    detail: `all ${checked} elements pass ${level.toUpperCase()}${capNote}`,
                  };
                }
                ...
                for (const el of document.querySelectorAll(
                  ...
                  if (rect.width < min || rect.height < min) {
                    const t = (el.textContent || el.getAttribute('aria-label') || '')
                      ...
                      .slice(0, 20);
                    small.push(
                      `${el.tagName.toLowerCase()} ${Math.round(rect.width)}x${Math.round(rect.height)}px "${t}"`,
                    );
                  }
                }
                if (small.length === 0)
                  return { assertion, pass: true, detail: `all ${checked} controls ≥ ${min}px` };
                return {
                  assertion,
                  pass: false,
                  detail: `${small.length}/${checked} controls < ${min}px: ${small.slice(0, 5).join('; ')}${small.length > 5 ? ` … and ${small.length - 5} more` : ''}`,
                };
              }
              ...
              if (lower === 'h1-count' || lower.startsWith('h1-count:')) {
                ...
                return {
                  assertion,
                  pass: h1s.length === want,
                  detail: `${h1s.length} h1 (expected ${want})${texts ? `: ${texts}` : ''}`,
                };
              }
              ...
              if (lower === 'heading-outline') {
                ...
                for (const h of hs) {
                  ...
                  if (prev) {
                    ...
                    if (level > prevLevel + 1) {
                      skips.push(
                        `h${prevLevel} "${(prev.textContent || '').trim().slice(0, 25)}" → h${level} "${(h.textContent || '').trim().slice(0, 25)}"`,
                      );
                    }
                  }
                  prev = h;
                ...
                if (skips.length === 0)
                  return { assertion, pass: true, detail: `${hs.length} headings, sequential` };
                return {
                  assertion,
                  pass: false,
                  detail: `${skips.length} skip(s): ${skips.slice(0, 3).join('; ')}`,
                };
              }
              ...
              if (lower === 'no-broken-images') {
                ...
                for (const img of imgs) {
                  ...
                  if (img.naturalWidth === 0 && (img.currentSrc || img.src)) {
                    broken.push((img.currentSrc || img.src).slice(0, 80));
                  }
                }
                if (broken.length === 0)
                  return { assertion, pass: true, detail: `${imgs.length} images ok` };
                return {
                  assertion,
                  pass: false,
                  detail: `${broken.length}/${imgs.length} broken: ${broken.slice(0, 5).join(', ')}`,
                };
              }
              ...
              if (lower === 'alt-text') {
                ...
                if (missing.length === 0)
                  return {
                    assertion,
                    pass: true,
                    detail: `all ${document.querySelectorAll('img').length} images have alt`,
                  };
                const names = missing
                  .slice(0, 5)
                  .map(
                ...
                return {
                  assertion,
                  pass: false,
                  detail: `${missing.length} image(s) without alt: ${names.join(', ')}`,
                };
              }
              ...
              if (lower === 'canonical') {
                ...
                return {
                  assertion,
                  pass: !!c?.href,
                  detail: c?.href ? c.href.slice(0, 80) : 'no <link rel="canonical">',
                };
              }
              if (lower === 'meta-description') {
                ...
                return {
                  assertion,
                  pass: d.length > 0,
                  detail: d ? `${d.length} chars` : 'no <meta name="description">',
                };
              }
              if (lower === 'og-image' || lower === 'og-title' || lower === 'og-tags') {
                ...
                const need =
                  lower === 'og-tags' ? ['title', 'description', 'image'] : [lower.slice(3)];
                const missing = need.filter((p) => !og(p));
                if (missing.length === 0)
                  return {
                    assertion,
                    pass: true,
                    detail: need.map((p) => `og:${p}=${og(p).slice(0, 40)}`).join(', '),
                  };
                return {
                  assertion,
                  pass: false,
                  detail: `missing ${missing.map((p) => `og:${p}`).join(', ')}`,
                };
              }
              if (lower === 'twitter-card') {
                ...
                return {
                  assertion,
                  pass: !!card,
                  detail: card ? `twitter:card=${card}` : 'no <meta name="twitter:card">',
                };
              }
              ...
              if (lower.startsWith('class:')) {
                ...
                try {
                  ...
                  return {
                    assertion,
                    pass: n > 0,
                    detail: `${n} element(s) with class containing "${name}"`,
                  };
                } catch {
                  return { assertion, pass: false, detail: 'invalid class pattern' };
                }
              }
              ...
              if (lower === 'translated') {
                ...
                if (!lang || lang.startsWith('en')) {
                  return {
                    assertion,
                    pass: true,
                    detail: `lang="${lang}" — skipped (English or no lang)`,
                  };
                }
                ...
                if (found.length > 0) {
                  return {
                    assertion,
                    pass: false,
                    detail: `${found.length} potential untranslated: ${found.join(', ')}`,
                  };
                }
                return {
                  assertion,
                  pass: true,
                  detail: `no common English phrases found (lang="${lang}")`,
                };
              }
  ...
  const results: CheckResult[] = items.map((item) => {
    ...
    if (!isKnownAssertion(item) || r.detail === 'unknown assertion') {
      return {
        ...
        pass: false,
        unknown: true,
        detail: `unknown assertion — known: ${knownAssertionHint()}`,
      };
    }
    return r;
  });
